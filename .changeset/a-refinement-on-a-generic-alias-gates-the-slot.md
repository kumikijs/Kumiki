...
it the same way, with the arguments substituted. The argument's own predicates
come first: `NonEmpty(Short)` over `type Short = Text where len-lt(7)` names
`len-lt(7)` for a value too long and `nonempty` for an empty one.
