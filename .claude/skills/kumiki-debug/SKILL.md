...
| `E0201` | a value does not have the type its position requires (slot init, assignment, `fn` argument / return, tile `in=`, operator operand) | correct the value, or widen the declared type. Two `nominal` types over one base are distinct, so a deliberate conversion goes through the base — a `fn` declared with the nominal return type takes a body of it |
