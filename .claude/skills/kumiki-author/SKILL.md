...
- **Two `nominal` types over one base are two types** (E0201): `postId := userId` is refused even when both are `nominal Text where uuid`, and so is **comparing** them — `postId == userId` and `cents < yen` are the same error, which is the shape a router or a lookup is written in (`if t.id == selectedProjectId`). The base still flows in and out — `slot c : Cents = 1`, `c := c + 1` and `c == 0` need no cast — so a deliberate conversion is a `fn` whose return type is the target *and whose body reaches the base*: `fn toUser(p: PostId) -> UserId = p + ""`. The identity body (`= p`) is the same E0201. Compare through that same `fn`: `t.id == toTaskId(pid)`, not `t.id == pid`. Minting an id is judged the same way — `TypeName.fresh()` is a `TypeName` and `TypeName.parse(t)` an `Option(TypeName)` — so `postId := UserId.fresh()` and `slot found : Option(PostId) = UserId.parse(t)` are both E0201. `parse` reads the text by the **base**, so `Cents.parse("12")` on a `nominal Int` is `Some(12)` and `Bool.parse("false")` is `Some(false)`. `Int` / `Float` read decimal text only (`" 12 "` and `"0x10"` are `None` — trim first), and a value that fails the type's `where` is `None` too. A type with no text reading (a record, a union, `File`) is E0802 — parse a base and build the value in a `fn`; a constructor without its arguments (`List.parse`, `Box.parse` for `type Box(T)`) is E0126 — name the application (`type IntBox = Box(Int)`).
