...
- **A test file lives inside a typechecked program.** Vitest strips types without checking them, so an assertion in an unchecked file can stop asserting without failing: `filter((d) => d.kind === "stale-closure-risk")` is structurally empty once that member is gone, and green forever. A package that ships tests declares a `typecheck` script whose config includes them — `tsconfig.typecheck.json` next to `tsconfig.json`, since the build's `rootDir` cannot take `test/` — and `packages/tests/typecheck-coverage.test.ts` fails when one does not.
