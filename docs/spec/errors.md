...
`()` is a value like any other: it is the one value of `Unit` ([stdlib §2.1](./stdlib.md#_2-1-built-in-types)), so it is accepted where `Unit` is declared and is this error everywhere else. `Card(())` against `tile Card in={label: Text}` reports `Expected {label: Text} but got Unit` at the `()` — it used to be accepted against every declared type, and the tile mounted with a `null` it then read `.label` from.
