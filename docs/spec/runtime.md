...
- **The served element tree is the one the client builds**, not only the same outer element: a tile whose renderer nests elements of its own is served nested the same way, because the alternative is a subtree hydration throws away and a crawler is served the wrong shape of.
  ...
  - A **closed** `modal` / `drawer` / `popover` is served as the present-but-hidden host the renderer mounts — `display: none` around the content box — rather than as an empty string. Opening a surface is a style flip on both paths, and the content of a closed one stays readable to a crawler.
  - `check` / `switch` / `radio` are served as the `<label>` that wraps their control, and `error` as the `<span>` its message will arrive in.
- Form state is the one thing a served attribute carries that the mounted element does not: `value`, `checked`, `selected`, `disabled`, `readonly` and `<details open>` are DOM **properties** on the client and attributes in the markup, which is the only way a served page can carry them.
