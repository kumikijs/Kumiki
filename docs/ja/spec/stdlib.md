...
フィールドは**ローカル**のものである。結果の文字列はタイムゾーンを含まないので読み手の壁時計として読まれる — UTC のフィールドを出すと、夕方が UTC の翌日にあたる読み手全員に前日が表示される。

`Time.parse` は §2.2.9 が `Time` に与えているのと同じ表現、すなわちミリ秒数を返す。時刻を指さないテキストは `None` になる。
