...
この節に書く名前はコンパイラが解決する：宣言済みの `theme` か宣言済みの slot でなければならず、それ以外は [E0118](./errors.md#e0118-undef-theme) になる。一方、その slot が保持する**値**は検査しない — 宣言された theme 名のいずれかであるべきだが、`app.start` でテーマを決めるアプリ（§4.6.1）は slot を何かの値から始めるほかなく、そこで誠実な初期値はどの theme も指さない値だからである。どの theme にも一致しない値は、組み込みの既定値で描画される。
