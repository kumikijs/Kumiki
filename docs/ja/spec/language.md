...
- **改行が文区切り**: `do=` 内のみ `;` で複数文
- **識別子は 32 文字以内**
- **入れ子は 256 段まで**: 自分自身を含みうる構文——括弧式・リスト・レコード・`if` の連鎖・tile 呼び出し・タプルパターン・型適用・theme レコード——はこれ以上深く入れ子にできない。パーサは再帰で下降するため、これを超えるとスタックを使い切って何も報告できなくなる。上限を設けることで、代わりに位置付きのパースエラーになる。実際に書かれるものがこの値に近づくことはない（examples と benchmarks 中の最深は 18 段）。前置演算子の連鎖（`not not x`）は入れ子ではなく、上限は無い。
- **複数行コメント禁止**
- **マクロ禁止**
