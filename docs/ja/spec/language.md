...
- **`Map(K, V)`** — そのキーのエントリ。`m[k] := v` はエントリを挿入または置換し、`m[k].f := v` はエントリのフィールドへ書き込む。
- **`List(T)`** — その位置の要素。`xs[i] := v` は同じ長さの新しい `List` の中で `i` の要素を置き換え、`xs[i].f := v` はその要素を通して書き込む。どの階層も形を保つ。要素を指さないインデックス — 末尾より先、負、整数でない `i` — は **何も書き込まない**。空の `.get` を通した書き込みと同じ no-op である：そこでは `xs.get(i)` が `None` を読むので、書き込みは読み取りと一致する。リストを伸ばすには `xs := xs.push(v)` と書く。
