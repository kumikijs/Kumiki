...
解決されるのは両側ともリテラルのみ。実行時に組み立てられる id（`{id: "row-" + t.id}`）は定義域に入らず、`for` 自体が式である場合は照合されない——[E0704](#e0704-unknown-icon) がアイコン名に対して採る「リテラルのみ」の方針と同じ。

**修正**：ラベルが指す id をコントロールに与えるか、名前を修正する。[フォーム](./forms.md) を参照。
