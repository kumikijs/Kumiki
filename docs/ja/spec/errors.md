...
同名の `reducer` 2 つが最も鋭いケースである：コード生成は両方を出力し、ランタイムは**最初**を dispatch する一方、型検査は**最後**しか検査していない。つまりツールチェーンの両半分が「どの定義が存在するか」について食い違っていた。
...
| `kind` | 対象 |
|---|---|
| `duplicate-clause` | `app` / `effect` の句（`caps = … caps = …`、`cap=… cap=…`） |
| `duplicate-key` | レコードリテラルのフィールド、リテラルの Map キー、`theme` / `motion` の項目（任意の深さ）、`app.routes` のルートパターン |
| `duplicate-field` | レコード**型**のフィールド（レコード型が書かれるあらゆる場所） |
| `duplicate-param` | `fn` の引数、`type` の型引数 |
