...
`nominal` はその例外であり、このコードの中で唯一、runtime なら動いたはずのプログラムを報告する規則である。nominal 型を同定するのは宣言された名前であり（[§1.3.5](./language.md#_1-3-5-型の一意化)）、同じ基底型に対する 2 つの宣言は互いを拒否する — `Cents := Yen`、`postId := userId`。一方で nominal とその基底型は双方向に受理し合うので、`slot c : Cents = 1` と `c := c + 1` は成立したままである。演算子はこの規則の外にある：`==` はすべての型に定義され、順序比較は両辺が数値かどうかだけを問うので、`cents < yen` は報告されない。
