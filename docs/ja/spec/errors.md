...
`()` も他の値と同じく扱われる。`()` は `Unit` の唯一の値であり（[stdlib §2.1](./stdlib.md#_2-1-ビルトイン型)）、`Unit` が宣言された位置では受け入れられ、それ以外ではすべてこのエラーになる。`tile Card in={label: Text}` に対する `Card(())` は `()` の位置で `Expected {label: Text} but got Unit` を報告する — 以前はどの宣言型に対しても受け入れられ、タイルは `null` を受け取ったままマウントして `.label` を読んでいた。
