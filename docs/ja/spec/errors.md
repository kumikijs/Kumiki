...
代入可能性は構造的で、暗黙変換は 1 つだけ — `Int` は `Float` の位置へ流れ、その逆は流れない。別名と generic の具体化は辿り、`where` の refinement は透過する：それは runtime の検査であり（[Forms §5.6](./forms.md#_5-6-バリデーション戦略)を参照）、`slot e : Email = "not-an-email"` はここでは受理されてバリデーションで失敗する。

`nominal` はその例外であり、このコードの中で唯一、runtime なら動いたはずのプログラムを報告する規則である。nominal 型を同定するのは宣言された名前であり（[§1.3.5](./language.md#_1-3-5-型の一意化)）、同じ基底型に対する 2 つの宣言は互いを拒否する — `Cents := Yen`、`postId := userId`。一方で nominal とその基底型は双方向に受理し合うので、`slot c : Cents = 1` と `c := c + 1` は成立したままである。演算子はこの規則の外にある：`==` はすべての型に定義され、順序比較は両辺が同じ族（number / text / time）に属するかだけを問う — 同じ基底型を持つ 2 つの nominal は常に属するので、`cents < yen` も `postId < userId` も報告されない。
...
**修正**：値を直すか、宣言型を広げる。nominal どうしの不一致が意図的なものであれば、基底型を経由して変換する — nominal を戻り型に宣言した `fn` はその基底型の body を受理し、これが言語の持つ構築形式である。
