...
| `function-identity` | 毎レンダ作り直される関数。codegen は reducer リストごとにクロージャを memo 化するのでコンパイル済みアプリからは出ない。inline でハンドラを作るホストは毎レンダ patch（patcher が無ければ再構築）を払う。ハンドラを memo 化すること。 |

対象範囲は `MountOptions.hostTileKinds` で決まり、パッケージエントリの `mount` は
`tiles` の上書きマップから導出する — built-in kind の上書きも含む。built-in レンダラを
差し替えた時点で、再利用を安全にしていたハンドラスロットも一緒に失われるからである。
