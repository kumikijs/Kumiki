...
ブラウザ上で Kumiki を編集 → コンパイル → プレビューできる。コンパイラ（`@kumikijs/compiler`）とランタイム（`@kumikijs/runtime`）はブラウザ内で動く。左で編集すると右に結果が出る。`例を選ぶ…` から [機能別カタログ](https://github.com/kumikijs/Kumiki/tree/main/packages/examples/features)の各例を読み込める。
...
ローカル CLI / エディタ統合用には、stdio で動く [`@kumikijs/mcp`](https://github.com/kumikijs/Kumiki/tree/main/packages/mcp) サーバーもある。
