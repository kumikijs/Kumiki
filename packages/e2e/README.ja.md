...
3 層検証（[docs/ja/spec/testing.md](../../docs/ja/spec/testing.md) §8.10）の中で、これは最も重いが最も忠実な層。日常は `kumiki check` / `kumiki smoke` / `kumiki run`（happy-dom、高速・CI 標準）で回し、フォーカス・レイアウト・実描画に関わるバグや最終確認のときにこの tier を使う。
